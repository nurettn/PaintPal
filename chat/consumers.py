...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def remove_user(self):
        await self.channel_layer.group_send(
            ...
            {"type": "delete_user", "user": self.nickname},
        )
    ...
    async def receive(self, text_data):
        ...
        if message_type == "stroke_coordinate":
            ...
            await self.channel_layer.group_send(
                self.room_group_name,
                {"type": "send_coordinate", "coordinate": coordinate},
            )
            ...
            await self.channel_layer.group_send(
                self.room_group_name,
                {"type": "prev_coordinate", "coordinate": coordinate},
            )

        elif message_type == "brushColor":
            color = text_data_json["color"]
            await self.channel_layer.group_send(
                self.room_group_name, {"type": "change_color", "color": color}
            )

        elif message_type == "strokeSize":
            size = text_data_json["size"]
            await self.channel_layer.group_send(
                self.room_group_name, {"type": "change_size", "size": size}
            )

        elif message_type == "clearCanvas":
            await self.channel_layer.group_send(
                self.room_group_name, {"type": "clear_canvas"}
            )
    ...
    async def prev_coordinate(self, event):
        ...
        await self.send(
            text_data=json.dumps(
                {"type": "prevCoordinate", "coordinate": coordinate}
            )
        )
    ...
    async def change_color(self, event):
        ...
        await self.send(
            text_data=json.dumps({"type": "brushColor", "color": color})
        )
    ...
    async def change_size(self, event):
        ...
        await self.send(
            text_data=json.dumps({"type": "change_size", "size": size})
        )
    ...
    async def update_playerlist(self, event):
        await self.send(
            text_data=json.dumps(
                {"type": "update_playerlist", "users": event["message"]}
            )
        )
